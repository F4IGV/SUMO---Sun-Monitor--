...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _set_panel_severity(self, key: str, family: str, sev: str, blink_on_danger: bool = True):
        """Apply a coherent accent color based on severity, with blink only when entering DANGER/EXTREME."""
        try:
            key = str(key)
        ...
        # Apply accent (no built-in blink here; we control it explicitly)
        p.set_accent(color, blink=False)

        # Blink border only when entering DANGER/EXTREME (RISK family)
        if family == "risk" and blink_on_danger:
            try:
                prev_i = _sev_index(RISK_ORDER, str(prev))
                now_i = _sev_index(RISK_ORDER, sev)
                entered_danger = (now_i >= _sev_index(RISK_ORDER, SEV_DANGER)) and (prev_i < _sev_index(RISK_ORDER, SEV_DANGER))
                if entered_danger:
...
# Dynamic Y zoom (based on recent finite values, clamped 0..100)
        ...
        p_aur.lbl_big.setText(
            f'<span style="color:{aur_accent}; font-size:21px; font-weight:900;">Max {_fmt_pct(aur_now)}</span>'
            f'&nbsp;&nbsp;'
            f'<span style="color:{ACCENT_BLUE}; font-size:21px; font-weight:900;">Mean {_fmt_pct(aur_mean)}</span>'
            f'&nbsp;&nbsp;'
            f'<span style="color:#aab6c5; font-size:21px; font-weight:900;">Area {_fmt_pct(aur_area)}</span>'
        )
