...
class KpLikePanel(QtWidgets.QFrame):
    ...
    def start_blink(self, duration_seconds: int = 300):
    ...
    def set_data(self, series: np.ndarray, big_value: float, y_range=None, color: str = None, x: np.ndarray | None = None):
        ...
        if getattr(self.cfg, "mode", "line") == "bar":
            ...
            self.plot.plot(x, series, pen=pg.mkPen(color, width=2))
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        configs = [
            ...
            ("AUR",  PanelConfig("AURORA GLOBAL ()", unit="%", big_value_fmt="{:.0f}")),
...
Note: this is an index, not a physical probability.""")
    ...
    def _on_data(self, d: dict):
        ...
        self.panels["KP"].set_accent(kp_accent)
        ...
        self.panels["SW"].set_accent(ACCENT_GREEN)
        self.panels["SW"].set_data(
            d.get("sw_speed_series", np.array([])),
            d.get("sw_speed_now", float("nan")),
            y_range=(0, 1200),
            color=ACCENT_GREEN,
        ...
        p_aur.lbl_title.setText("AURORA GLOBAL ()")
...
# Dynamic Y zoom (based on recent finite values, clamped 0..100)
        ...
        p_aur.lbl_big.setText(
            f'<span style="color:{ACCENT_GREEN}; font-weight:900;">Max {_fmt_pct(aur_now)}</span>'
            f'&nbsp;&nbsp;'
            f'<span style="color:{ACCENT_BLUE}; font-weight:900;">Mean {_fmt_pct(aur_mean)}</span>'
            f'&nbsp;&nbsp;'
            f'<span style="color:{STATUS_WARN}; font-weight:900;">Area {_fmt_pct(aur_area)}</span>'
        )
