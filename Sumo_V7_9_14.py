...
def find_sumo_icon_path() -> Path | None:
    """Find the best SUMO window/app icon in bundled or development assets.

    Priority is given to .ico files for Windows taskbar/ALT-TAB support.
    PNG/SVG are accepted for the Qt window icon when no ICO is present.
    """
    candidates = [
        resource_path("assets/icon.ico"),
        resource_path("assets/sumo.ico"),
        resource_path("assets/logo.ico"),
        resource_path("icon.ico"),
        resource_path("sumo.ico"),
        resource_path("assets/icon.png"),
        resource_path("assets/sumo.png"),
...
def build_sumo_icon() -> QtGui.QIcon:
    """Build a QIcon for SUMO. Returns an empty icon if no asset is available."""
    try:
        icon_path = find_sumo_icon_path()
        if icon_path is None:
            debug_log("SUMO icon not found in assets; using default Qt/Windows icon")
            return QtGui.QIcon()
        icon = QtGui.QIcon(str(icon_path))
        if icon.isNull():
            debug_log(f"SUMO icon found but Qt could not load it: {icon_path}")
            return QtGui.QIcon()
        debug_log(f"SUMO icon loaded: {icon_path}")
        return icon
    except Exception as exc:
        debug_log(f"SUMO icon load error: {exc}")
        return QtGui.QIcon()
...
DAPNET_GLOBAL_COOLDOWN = 120  # seconds, global anti-spam guard for non-ISS DAPNET modules
DAPNET_ISS_COOLDOWN = 30  # seconds, allows AOS/peak/LOS during short ISS passes
...
def normalize_dapnet_config(cfg: Optional[dict]) -> dict:
    ...
    mf = base.get("meteo_france", {})
    mf["enabled"] = bool(mf.get("enabled", False))
    mf["api_token"] = str(mf.get("api_token") or "").strip()
    try:
        mf["token_created_ts"] = int(float(mf.get("token_created_ts", 0) or 0))
    except Exception:
        mf["token_created_ts"] = 0
    try:
        mf["token_validity_seconds"] = max(0, int(float(mf.get("token_validity_seconds", 31536000) or 0)))
    except Exception:
        mf["token_validity_seconds"] = 31536000
    mf["api_url"] = str(mf.get("api_url") or "https://public-api.meteofrance.fr/public/DPVigilance/v1/cartevigilance/encours").strip()
    mf["departments"] = str(mf.get("departments") or "35").strip()
    try:
        mf["refresh_minutes"] = max(5, int(mf.get("refresh_minutes", 5)))
    except Exception:
        mf["refresh_minutes"] = 5
    mf["emergency_on_red"] = bool(mf.get("emergency_on_red", True))
    allowed_levels = {"disabled", "yellow", "orange", "red"}
    default_ph = default_dapnet_config().get("meteo_france", {}).get("phenomena", {})
    ph = dict(default_ph)
    if isinstance(mf.get("phenomena"), dict):
        ph.update(mf.get("phenomena") or {})
    mf["phenomena"] = {str(k): (str(v).strip().lower() if str(v).strip().lower() in allowed_levels else str(default_ph.get(k, "orange"))) for k, v in ph.items()}
...
# =====================================================
# METEO-FRANCE VIGILANCE -> DAPNET
# =====================================================
METEO_FRANCE_LEVELS = {"green": 1, "yellow": 2, "orange": 3, "red": 4}
METEO_FRANCE_LEVEL_NAMES = {1: "VERT", 2: "JAUNE", 3: "ORANGE", 4: "ROUGE"}
METEO_FRANCE_PHENOMENA = {
    "wind": "Vent violent",
...
METEO_FRANCE_PHENOMENA_ALIASES = {
    "1": "wind", "vent": "wind", "vent violent": "wind", "wind": "wind",
    "2": "rain_flood", "pluie": "rain_flood", "pluie-inondation": "rain_flood", "rain": "rain_flood",
    "3": "thunderstorm", "orage": "thunderstorm", "orages": "thunderstorm", "thunderstorm": "thunderstorm",
    "4": "flood", "crue": "flood", "crues": "flood", "flood": "flood",
    "5": "snow_ice", "neige": "snow_ice", "neige-verglas": "snow_ice", "verglas": "snow_ice", "snow": "snow_ice",
    "6": "heatwave", "canicule": "heatwave", "heatwave": "heatwave",
    "7": "cold", "grand froid": "cold", "froid": "cold", "cold": "cold",
    "8": "avalanche", "avalanches": "avalanche",
    "9": "coastal", "vagues-submersion": "coastal", "submersion": "coastal", "coastal": "coastal",
}

def _mf_normalize_department(value) -> str:
    s = str(value or "").strip().upper()
    if not s:
        return ""
    if s in ("2A", "2B"):
        return s
    m = re.search(r"(2A|2B|\d{1,3})", s)
    if not m:
        return s
...
def _mf_parse_departments(value) -> set[str]:
    if isinstance(value, (list, tuple, set)):
        raw = []
        for x in value:
            raw.extend(str(x).replace(";", ",").split(","))
    else:
        raw = re.split(r"[,;\s]+", str(value or ""))
    return {d for d in (_mf_normalize_department(x) for x in raw) if d}

def _mf_level_to_int(value) -> int:
    try:
        if value is None:
            return 0
        ...
        if s.isdigit():
            return int(s)
        if "rouge" in s or "red" in s:
            return 4
        if "orange" in s:
            return 3
        if "jaune" in s or "yellow" in s:
            return 2
        if "vert" in s or "green" in s:
            return 1
    except Exception:
        pass
    return 0

def _mf_level_name(level: int) -> str:
    return METEO_FRANCE_LEVEL_NAMES.get(int(level), "?")

def _mf_threshold_to_int(value) -> int:
    return {"disabled": 99, "green": 1, "yellow": 2, "orange": 3, "red": 4}.get(str(value or "orange").strip().lower(), 3)

def _mf_normalize_phenomenon(value) -> str:
    s = str(value or "").strip().lower()
    s = s.replace("é", "e").replace("è", "e").replace("ê", "e").replace("à", "a")
    s = s.replace("_", "-").replace("/", "-")
    return METEO_FRANCE_PHENOMENA_ALIASES.get(s, METEO_FRANCE_PHENOMENA_ALIASES.get(s.split("-")[0], s))


def _mf_ascii_token(value) -> str:
    """Normalize a short Météo-France token without depending on unicodedata."""
    s = str(value or "").strip().lower()
    repl = {
        "é": "e", "è": "e", "ê": "e", "ë": "e",
        "à": "a", "â": "a", "ä": "a",
...
def _mf_normalize_deadline(value) -> str:
    """Return J0/J+1 when the Vigilance JSON exposes an échéance.

    Météo-France/data.gouv mirrors have used several shapes over time:
    - echeance/échéance: J, J0, J1, J+1, 0, 1
    - nested timelaps/echeances maps whose key is the deadline
    - natural labels such as immediate/courant/demain.
    Empty/unknown is kept as J0 to avoid losing current-alert compatibility.
    """
    try:
        if value in (None, ""):
            return "J0"
        if isinstance(value, (int, float)):
            ...
            if n == 1:
                return "J+1"
            return f"J+{n}"
        s = _mf_ascii_token(value)
        if not s:
            return "J0"
        s_flat = s.replace("+", "").replace("-", "")
        if s in {"j", "j0", "j-0", "0", "now", "current", "courant", "courante", "immediat", "immediate", "aujourdhui", "today"}:
            return "J0"
        if s in {"j1", "j+1", "j-1", "1", "demain", "tomorrow", "lendemain"} or s_flat == "j1":
            return "J+1"
        m = re.search(r"(?:j|jour)?\+?(\d+)$", s_flat)
        if m:
            n = int(m.group(1))
            return "J0" if n <= 0 else f"J+{n}"
        if "demain" in s or "tomorrow" in s:
            return "J+1"
        if "immed" in s or "courant" in s or "today" in s:
            return "J0"
    except Exception:
        pass
    return "J0"
...
def _mf_extract_deadline_from_dict(obj: dict, default: str = "J0") -> str:
    try:
        for k, v in obj.items():
            key = _mf_ascii_token(k)
            if key in {"echeance", "deadline", "timelap", "timelaps", "time-lap", "time-laps", "validity-day", "jour", "day"}:
                if not isinstance(v, (dict, list)):
                    return _mf_normalize_deadline(v)
        return default
    except Exception:
        return default

def _mf_extract_vigilances(payload) -> list[dict]:
    """Extract dep/phenomenon/level/deadline triplets from Vigilance JSON.

    The official schema and data.gouv mirrors have evolved over time, so this
    parser is deliberately tolerant. The important addition is `deadline`:
    J0 = prévision immédiate / aujourd'hui, J+1 = prévision du lendemain.
    """
    out: list[dict] = []

    def walk(obj, context=None):
        context = dict(context or {})
        if isinstance(obj, dict):
            local = dict(context)

            # Échéance first, because some schemas put it at parent level.
            local["deadline"] = _mf_extract_deadline_from_dict(obj, local.get("deadline", "J0"))

            # Do NOT treat "echeance" as a department. Older SUMO did this and
            # could lose the J0/J+1 information in DAPNET messages.
            for dep_key in ("dep", "dept", "department", "department_code", "domain_id", "id", "code", "area_id"):
                if dep_key in obj:
                    d = _mf_normalize_department(obj.get(dep_key))
                    if d:
                        local["dep"] = d
                        break

            for ph_key in ("phenomenon", "phenomene", "phenomenon_id", "phenomene_id", "phenomenonId", "phenomenonName", "risk_name", "libelle"):
                if ph_key in obj:
                    ph = _mf_normalize_phenomenon(obj.get(ph_key))
                    if ph:
                        local["phenomenon"] = ph
                        break

            for lv_key in ("color", "color_id", "couleur", "niveau", "risk_level", "level", "vigilance_level", "phenomenon_max_color_id"):
                if lv_key in obj:
                    lv = _mf_level_to_int(obj.get(lv_key))
                    if lv:
                        local["level"] = lv
                        break

            if local.get("dep") and local.get("phenomenon") and local.get("level"):
                out.append(dict(local))

            # Common maps: risks: {"3": 2} or {"thunderstorm": "orange"}.
            for map_key in ("risks", "risk", "phenomenons", "phenomena", "vigilances"):
                val = obj.get(map_key)
                if isinstance(val, dict):
                    for k, v in val.items():
                        ph = _mf_normalize_phenomenon(k)
                        if isinstance(v, dict):
                            walk(v, {**local, "phenomenon": ph})
                        else:
                            lv = _mf_level_to_int(v)
                            if local.get("dep") and ph and lv:
                                out.append({**local, "phenomenon": ph, "level": lv})
                elif isinstance(val, list):
                    for item in val:
                        walk(item, local)

            # Common deadline maps: timelaps/echeances: {"J": [...], "J1": [...]}.
            for map_key in ("timelaps", "timelap", "time_laps", "time_lap", "echeances", "echeance", "échéances", "échéance"):
                val = obj.get(map_key)
                if isinstance(val, dict):
                    for k, v in val.items():
                        dl = _mf_normalize_deadline(k)
                        walk(v, {**local, "deadline": dl})
                elif isinstance(val, list):
                    for item in val:
                        walk(item, local)
    ...
    # De-duplicate and keep highest level per dep/phenom/deadline.
    best: dict[tuple[str, str, str], dict] = {}
    for item in out:
        dep = _mf_normalize_department(item.get("dep"))
        ph = _mf_normalize_phenomenon(item.get("phenomenon"))
        lv = _mf_level_to_int(item.get("level"))
        deadline = _mf_normalize_deadline(item.get("deadline", "J0"))
        if not dep or ph not in METEO_FRANCE_PHENOMENA or lv <= 1:
            continue
        key = (dep, ph, deadline)
        if key not in best or lv > int(best[key].get("level", 0)):
            best[key] = {"dep": dep, "phenomenon": ph, "level": lv, "deadline": deadline}
    return list(best.values())

def _mf_parse_any_datetime(value) -> Optional[datetime]:
    try:
        if value in (None, ""):
            return None
        if isinstance(value, (int, float)):
        ...
        if not s:
            return None
        # Try HTTP date / RFC style first.
        dt_http = parse_http_date(s)
        if dt_http is not None:
            return dt_http.astimezone(timezone.utc)
        # Try ISO and Météo-France common formats.
...
def _mf_find_bulletin_timestamp(payload) -> Optional[datetime]:
    """Best-effort extraction of a bulletin/publication timestamp from Vigilance JSON."""
    timestamp_keys = {
        "timestamp", "time", "datetime", "date", "dateprod", "date_prod",
        "publication_date", "publicationdate", "published", "published_at",
        "updated", "updated_at", "update_time", "last_update", "date_maj",
        "datemaj", "bulletin_date", "bulletindate", "generated_at",
        "product_datetime", "validity_start", "valid_from", "begin_validity_date",
    }
    best: Optional[datetime] = None

    def consider(value) -> None:
        nonlocal best
        dt = _mf_parse_any_datetime(value)
        if dt is None:
            return
        # Avoid clearly bogus old dates.
        try:
            if dt.year < 2000:
                return
        except Exception:
    ...
    def walk(obj):
        if isinstance(obj, dict):
            for k, v in obj.items():
                ...
                if key in timestamp_keys or "date" in key or "time" in key or "valid" in key:
                    if not isinstance(v, (dict, list)):
                        consider(v)
                if isinstance(v, (dict, list)):
                    walk(v)
...
def _mf_count_departments(events: list[dict]) -> int:
    try:
        return len({ _mf_normalize_department(ev.get("dep")) for ev in events if _mf_normalize_department(ev.get("dep")) })
    except Exception:
        return 0


def _mf_fetch_json_detailed(cfg: dict, logger=None) -> tuple[dict, dict]:
    """Download and validate Météo-France JSON with detailed status for UI/debug."""
    url = str(cfg.get("api_url") or "").strip()
    if not url:
        raise RuntimeError("Météo-France API URL missing")
    headers = {"User-Agent": "SUMO-SunMonitor/7.9.8", "Accept": "application/json"}
    ...
    log(f"[MeteoFrance] download start url={url}")
    try:
        if NETWORK_FETCH_SEMAPHORE is not None:
            with NETWORK_FETCH_SEMAPHORE:
            ...
            r = requests.get(url, timeout=20, headers=headers)
        status["http_code"] = int(getattr(r, "status_code", 0) or 0)
        status["content_type"] = str(r.headers.get("Content-Type", ""))
        raw = getattr(r, "content", b"") or b""
        status["bytes"] = len(raw)
        log(f"[MeteoFrance] HTTP {status['http_code']} bytes={status['bytes']} content_type={status['content_type']}")
        r.raise_for_status()
        payload = r.json()
        status["json_ok"] = True
        events = _mf_extract_vigilances(payload)
        bulletin_dt = _mf_find_bulletin_timestamp(payload)
        status["events_count"] = int(len(events))
        status["departments_count"] = int(_mf_count_departments(events))
        status["bulletin_ts"] = bulletin_dt.isoformat() if bulletin_dt else ""
        status["ok"] = True
        log(
            "[MeteoFrance] JSON parse OK "
            f"events={status['events_count']} departments={status['departments_count']} "
            f"bulletin_ts={status['bulletin_ts'] or '?'}"
        )
        return payload, status
    except Exception as exc:
        status["error"] = str(exc)
        log(f"[MeteoFrance] ERROR {exc}")
        raise


def _mf_fetch_json(cfg: dict) -> dict:
    payload, _status = _mf_fetch_json_detailed(cfg)
    return payload



def _mf_format_local_utc_ts(ts_value) -> str:
    try:
        ts = int(float(ts_value or 0))
        if ts <= 0:
            return "not set"
...
def _mf_format_duration_short(seconds_value) -> str:
    try:
...
def _mf_token_validity_label(seconds_value) -> str:
    try:
        seconds = int(float(seconds_value or 0))
    except Exception:
        seconds = 0
    for label, value in MF_TOKEN_VALIDITY_PRESETS:
        if value >= 0 and int(value) == seconds:
            return f"{label} ({value} seconds)"
    if seconds > 0:
        return f"Custom ({seconds} seconds)"
    return "Custom"

def _mf_token_status_text(mf_cfg: dict) -> str:
    try:
        token = str((mf_cfg or {}).get("api_token") or "").strip()
        created_ts = int(float((mf_cfg or {}).get("token_created_ts", 0) or 0))
        validity = int(float((mf_cfg or {}).get("token_validity_seconds", 0) or 0))
        if not token:
            return "🔴 Token missing"
        if created_ts <= 0:
            return "🟡 Token present, creation date not set"
        if validity <= 0:
            return "🟡 Token present, validity unknown"
        expires_ts = created_ts + validity
        now_ts = int(datetime.now(timezone.utc).timestamp())
        remaining = expires_ts - now_ts
        created_txt = _mf_format_local_utc_ts(created_ts)
        expires_txt = _mf_format_local_utc_ts(expires_ts)
        if remaining <= 0:
            return f"🔴 Token expired\nCreated: {created_txt}\nExpired: {expires_txt}"
        if remaining < 30 * 86400:
            return f"🟡 Token expires soon\nCreated: {created_txt}\nExpires: {expires_txt}\nRemaining: {_mf_format_duration_short(remaining)}"
        return f"🟢 Token valid\nCreated: {created_txt}\nExpires: {expires_txt}\nRemaining: {_mf_format_duration_short(remaining)}"
    except Exception as exc:
        return f"Token status unavailable: {exc}"


def _mf_status_from_state() -> str:
    try:
        state = load_dapnet_state(DAPNET_STATE_PATH)
        ok_ts = str(state.get("meteo_france_last_success_utc") or "")
        check_ts = str(state.get("meteo_france_last_check_utc") or "")
        http_code = state.get("meteo_france_last_http_code", "?")
        json_ok = bool(state.get("meteo_france_last_json_ok", False))
        bulletin = str(state.get("meteo_france_last_bulletin_utc") or "")
        events = state.get("meteo_france_last_events_count", "?")
        deps = state.get("meteo_france_last_departments_count", "?")
        err = str(state.get("meteo_france_last_error") or "").strip()
        if ok_ts and json_ok:
            return (
                f"OK • HTTP {http_code} • JSON OK • events={events} • deps={deps}\n"
                f"Last download: {ok_ts}\n"
                f"Bulletin timestamp: {bulletin or '?'}"
            )
        if check_ts:
            return f"Last check: {check_ts} • HTTP {http_code} • ERROR: {err or 'unknown'}"
        return "No Météo-France API check yet."
    except Exception as exc:
...
class DapnetClient:
    ...
    def send_message(self, text: str, emergency: bool = False, force: bool = False, recipients: Optional[list[str]] = None, tx_group: Optional[str] = None, cooldown_seconds: Optional[int] = None) -> tuple[bool, str]:
        if not force:
        ...
        now_ts = time.time()
        try:
            effective_cooldown = DAPNET_GLOBAL_COOLDOWN if cooldown_seconds is None else max(0, int(cooldown_seconds))
        except Exception:
            effective_cooldown = DAPNET_GLOBAL_COOLDOWN
        if effective_cooldown > 0 and now_ts - float(_last_dapnet_global or 0.0) < effective_cooldown:
            remaining = int(effective_cooldown - (now_ts - float(_last_dapnet_global or 0.0)))
    ...
    def _prepare_iss_tle(self):
        ...
        def _worker():
            ...
            try:
                ...
                remote_urls = []
                try:
                    primary_url = str(getattr(self, "_iss_tle_url", ISS_TLE_URL) or ISS_TLE_URL).strip()
                    if primary_url:
                        remote_urls.append(primary_url)
                except Exception:
                    remote_urls.append(ISS_TLE_URL)
                try:
                    for u in ISS_TLE_FALLBACK_URLS:
                        if u and u not in remote_urls:
                            remote_urls.append(u)
                except Exception:
                    pass

                # CelesTrak's legacy stations.txt can answer 403 depending on the client/path.
                # Try the current GP endpoint first, then legacy, with a browser-like UA.
                for attempt in range(1, 4):
                    for remote_url in remote_urls:
                        try:
                            debug_log(f"ISS TLE remote fetch attempt={attempt} url={remote_url}")
                            remote_blob = safe_get_text(
                                remote_url,
                                timeout=20,
                                headers={
                                    "User-Agent": "SUMO-SunMonitor/7.9.9 (+https://hamtech.fr; F4IGV)",
                            ...
                            sat, epoch_text, loaded_from = _build_satellite(remote_parsed, f'remote refresh • {remote_url}')
                            err = ""
                            remote_error = ""
                            self._iss_tle_url = remote_url
                            debug_log(f"ISS TLE remote load OK: {epoch_text}")
                            break
                        except Exception as e:
                            remote_error = str(e)
                            debug_log(f"ISS TLE remote fetch failed attempt={attempt} url={remote_url}: {remote_error}")
                    if sat is not None and not remote_error:
                        break
...
class KC2GRemoteMufMapWidget(QtWidgets.QWidget):
    ...
    def reload(self):
        try:
            ...
            if not hasattr(self, "instrument"):
                if getattr(self, "_stopped", False) or not getattr(self, "_alive", True):
                    return
                if getattr(self, "_fetch_in_progress", False):
                    debug_log("KC2GRemoteMufMapWidget.reload skipped reason=in_progress")
                    return
    ...
    def _start_fetch_worker(self, generation: int):
        """Start the KC2G SVG fetcher in its own QThread.

        This method was referenced by reload() but missing in v7.9.8, which
        produced: object has no attribute '_start_fetch_worker'.
        """
        if getattr(self, "_stopped", False) or not getattr(self, "_alive", True) or not _qt_obj_alive(self):
            return
        if getattr(self, "_fetch_in_progress", False):
            debug_log(f"KC2GRemoteMufMapWidget reload skipped gen={generation} reason=in_progress")
            return

        self._fetch_generation = int(generation)
        self._fetch_in_progress = True
        self._loading = True
        self._status_text = "Loading KC2G MUF map…"

        thread = QtCore.QThread(self)
        worker = KC2GRemoteMapFetchWorker(self.url, timeout=20, generation=int(generation))
        ...
        thread.finished.connect(lambda g=int(generation): debug_log(f"KC2GRemoteMufMapWidget fetch thread finished gen={g}"))
        thread.start()
...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(
        ...
        auto_focus_solar_on_alert: bool = True,
        auto_focus_min_severity: str = "ALERT",
        auto_focus_hold_seconds: int = 60,
        auto_focus_cooldown_minutes: int = 10,
        muf_source: str = "sumo",
        ...
        self.cb_auto_focus_solar_on_alert = QtWidgets.QCheckBox("Return to solar tiles when a solar/weather-space alert appears")
        self.cb_auto_focus_solar_on_alert.setChecked(bool(auto_focus_solar_on_alert))
        self.cb_auto_focus_solar_on_alert.setToolTip("If SUMO is showing another dashboard, it temporarily returns to the 9 solar-weather tiles when a risk panel reaches the chosen severity.")
        form_display.addRow("Alert focus:", self.cb_auto_focus_solar_on_alert)

        self.cb_auto_focus_min_severity = QtWidgets.QComboBox()
        for label, data in (("WATCH", "WATCH"), ("ALERT", "ALERT"), ("DANGER", "DANGER"), ("EXTREME", "EXTREME")):
            self.cb_auto_focus_min_severity.addItem(label, data)
        _afs = str(auto_focus_min_severity or "ALERT").strip().upper()
        if _afs not in ("WATCH", "ALERT", "DANGER", "EXTREME"):
            _afs = "ALERT"
        self.cb_auto_focus_min_severity.setCurrentIndex(max(0, self.cb_auto_focus_min_severity.findData(_afs)))
        self.cb_auto_focus_min_severity.setToolTip("Minimum risk level required to force the display back to the 9 solar-weather tiles.")
        form_display.addRow("Alert focus min level:", self.cb_auto_focus_min_severity)

        self.sb_auto_focus_hold_seconds = QtWidgets.QSpinBox()
        self.sb_auto_focus_hold_seconds.setRange(10, 3600)
        self.sb_auto_focus_hold_seconds.setSuffix(" s")
        self.sb_auto_focus_hold_seconds.setValue(max(10, int(auto_focus_hold_seconds or 60)))
        self.sb_auto_focus_hold_seconds.setToolTip("How long SUMO keeps the solar tiles visible after an alert focus switch.")
        form_display.addRow("Keep solar visible:", self.sb_auto_focus_hold_seconds)

        self.sb_auto_focus_cooldown_minutes = QtWidgets.QSpinBox()
        self.sb_auto_focus_cooldown_minutes.setRange(1, 240)
        self.sb_auto_focus_cooldown_minutes.setSuffix(" min")
        self.sb_auto_focus_cooldown_minutes.setValue(max(1, int(auto_focus_cooldown_minutes or 10)))
        self.sb_auto_focus_cooldown_minutes.setToolTip("Anti-spam delay before a new alert can force the display again.")
        form_display.addRow("Focus cooldown:", self.sb_auto_focus_cooldown_minutes)
        ...
        hint_display = QtWidgets.QLabel(
            ...
            "• Alert focus can temporarily bring SUMO back to the 9 solar-weather tiles when an alert appears."
        )
        ...
        # ========== TAB: Météo-France Vigilance ==========
        tab_meteo = QtWidgets.QWidget()
        met_l = QtWidgets.QVBoxLayout(tab_meteo)
        met_l.setContentsMargins(14, 14, 14, 14)
        met_l.setSpacing(10)

        mf_cfg = self._dapnet_cfg.get("meteo_france", {})
        gb_mf_main = QtWidgets.QGroupBox("Météo-France Vigilance → DAPNET")
        gb_mf_main_l = QtWidgets.QFormLayout(gb_mf_main)

        self.cb_mf_enabled = QtWidgets.QCheckBox("Enable Météo-France Vigilance alerts")
        self.cb_mf_enabled.setChecked(bool(mf_cfg.get("enabled", False)))
        gb_mf_main_l.addRow("Module:", self.cb_mf_enabled)

        self.ed_mf_departments = QtWidgets.QLineEdit(str(mf_cfg.get("departments") or "35"))
        self.ed_mf_departments.setPlaceholderText("35, 53, 29, 2A...")
        self.ed_mf_departments.setClearButtonEnabled(True)
        gb_mf_main_l.addRow("Departments:", self.ed_mf_departments)

        self.sb_mf_refresh = QtWidgets.QSpinBox()
        self.sb_mf_refresh.setRange(5, 180)
        self.sb_mf_refresh.setSuffix(" min")
        self.sb_mf_refresh.setValue(int(mf_cfg.get("refresh_minutes", 5)))
        gb_mf_main_l.addRow("Refresh:", self.sb_mf_refresh)

        self.cb_mf_emergency_red = QtWidgets.QCheckBox("Use DAPNET emergency flag for RED alerts")
        self.cb_mf_emergency_red.setChecked(bool(mf_cfg.get("emergency_on_red", True)))
        gb_mf_main_l.addRow("Emergency:", self.cb_mf_emergency_red)

        self.ed_mf_api_token = QtWidgets.QLineEdit(str(mf_cfg.get("api_token") or ""))
        self.ed_mf_api_token.setPlaceholderText("Météo-France API token / Bearer token")
        self.ed_mf_api_token.setEchoMode(QtWidgets.QLineEdit.Password)
        self.ed_mf_api_token.setClearButtonEnabled(True)
        mf_token_row = QtWidgets.QHBoxLayout()
        mf_token_row.addWidget(self.ed_mf_api_token, 1)
        self.btn_mf_get_token = QtWidgets.QPushButton("Get API Token")
        self.btn_mf_get_token.setToolTip("Open the official Météo-France API portal to generate a token")
        self.btn_mf_get_token.clicked.connect(self._open_meteo_france_api_portal)
        mf_token_row.addWidget(self.btn_mf_get_token)
        gb_mf_main_l.addRow("API token:", mf_token_row)

        current_mf_validity = int(mf_cfg.get("token_validity_seconds", 31536000) or 0)
        self.cb_mf_token_validity_preset = QtWidgets.QComboBox()
        self.cb_mf_token_validity_preset.setToolTip("Choose the same validity duration when generating the token on the Météo-France API portal.")
        for label, seconds in MF_TOKEN_VALIDITY_PRESETS:
            if seconds >= 0:
                self.cb_mf_token_validity_preset.addItem(f"{label} — {seconds} seconds", int(seconds))
            else:
                self.cb_mf_token_validity_preset.addItem(label, int(seconds))
        preset_index = self.cb_mf_token_validity_preset.findData(current_mf_validity)
        if preset_index < 0:
            preset_index = self.cb_mf_token_validity_preset.findData(-1)
        self.cb_mf_token_validity_preset.setCurrentIndex(max(0, preset_index))
        gb_mf_main_l.addRow("Token validity preset:", self.cb_mf_token_validity_preset)

        self.sb_mf_token_validity = QtWidgets.QSpinBox()
        self.sb_mf_token_validity.setRange(0, 2147483647)
        self.sb_mf_token_validity.setSingleStep(86400)
        self.sb_mf_token_validity.setValue(current_mf_validity)
        self.sb_mf_token_validity.setToolTip("Copy this value into the Météo-France token validity field. You can edit it for a custom duration.")
        gb_mf_main_l.addRow("Validity seconds to copy:", self.sb_mf_token_validity)

        self.ed_mf_token_created = QtWidgets.QLineEdit(_mf_format_local_utc_ts(mf_cfg.get("token_created_ts", 0)))
        self.ed_mf_token_created.setReadOnly(True)
        self.ed_mf_token_created.setStyleSheet("color: #d7dde6;")
        mf_created_row = QtWidgets.QHBoxLayout()
        mf_created_row.addWidget(self.ed_mf_token_created, 1)
        self.btn_mf_token_set_now = QtWidgets.QPushButton("Set now")
        self.btn_mf_token_set_now.setToolTip("Set token creation date to current UTC time")
        mf_created_row.addWidget(self.btn_mf_token_set_now)
        gb_mf_main_l.addRow("Token created:", mf_created_row)

        self.lb_mf_token_status = QtWidgets.QLabel(_mf_token_status_text(mf_cfg))
        self.lb_mf_token_status.setWordWrap(True)
        self.lb_mf_token_status.setStyleSheet("color: #aab6c5; font-size: 11px;")
        gb_mf_main_l.addRow("Token status:", self.lb_mf_token_status)

        self._mf_token_created_ts = int(mf_cfg.get("token_created_ts", 0) or 0)
        def _refresh_mf_token_status_ui():
            try:
                tmp_cfg = {
                    "api_token": self.ed_mf_api_token.text().strip(),
                    "token_created_ts": int(getattr(self, "_mf_token_created_ts", 0) or 0),
                    "token_validity_seconds": int(self.sb_mf_token_validity.value()),
                }
                self.ed_mf_token_created.setText(_mf_format_local_utc_ts(tmp_cfg["token_created_ts"]))
                self.lb_mf_token_status.setText(_mf_token_status_text(tmp_cfg) + "\nSelected validity: " + _mf_token_validity_label(tmp_cfg["token_validity_seconds"]))
            except Exception as exc:
                self.lb_mf_token_status.setText(f"Token status unavailable: {exc}")
        self._refresh_mf_token_status_ui = _refresh_mf_token_status_ui

        def _set_mf_token_now():
            try:
                self._mf_token_created_ts = int(datetime.now(timezone.utc).timestamp())
                self._refresh_mf_token_status_ui()
            except Exception:
                pass
        def _on_mf_validity_preset_changed(index):
            try:
                seconds = int(self.cb_mf_token_validity_preset.itemData(index) or -1)
                if seconds >= 0 and self.sb_mf_token_validity.value() != seconds:
                    self.sb_mf_token_validity.setValue(seconds)
                self._refresh_mf_token_status_ui()
            except Exception:
                pass

        def _on_mf_validity_seconds_changed(value):
            try:
                idx = self.cb_mf_token_validity_preset.findData(int(value))
                if idx < 0:
                    idx = self.cb_mf_token_validity_preset.findData(-1)
                if idx >= 0 and self.cb_mf_token_validity_preset.currentIndex() != idx:
                    self.cb_mf_token_validity_preset.blockSignals(True)
                    self.cb_mf_token_validity_preset.setCurrentIndex(idx)
                    self.cb_mf_token_validity_preset.blockSignals(False)
                self._refresh_mf_token_status_ui()
            except Exception:
                pass

        self.btn_mf_token_set_now.clicked.connect(_set_mf_token_now)
        self.ed_mf_api_token.textChanged.connect(lambda *_: self._refresh_mf_token_status_ui())
        self.cb_mf_token_validity_preset.currentIndexChanged.connect(_on_mf_validity_preset_changed)
        self.sb_mf_token_validity.valueChanged.connect(_on_mf_validity_seconds_changed)

        self.ed_mf_api_url = QtWidgets.QLineEdit(str(mf_cfg.get("api_url") or "https://public-api.meteofrance.fr/public/DPVigilance/v1/cartevigilance/encours"))
        self.ed_mf_api_url.setClearButtonEnabled(True)
        gb_mf_main_l.addRow("API URL:", self.ed_mf_api_url)

        self.lb_mf_api_status = QtWidgets.QLabel(_mf_status_from_state())
        self.lb_mf_api_status.setWordWrap(True)
        self.lb_mf_api_status.setStyleSheet("color: #aab6c5; font-size: 11px;")
        gb_mf_main_l.addRow("API status:", self.lb_mf_api_status)
        met_l.addWidget(gb_mf_main)

        gb_mf_ph = QtWidgets.QGroupBox("Minimum level by phenomenon")
        gb_mf_ph_l = QtWidgets.QGridLayout(gb_mf_ph)
        gb_mf_ph_l.setHorizontalSpacing(10)
        gb_mf_ph_l.setVerticalSpacing(6)
        gb_mf_ph_l.addWidget(QtWidgets.QLabel("Phenomenon"), 0, 0)
        gb_mf_ph_l.addWidget(QtWidgets.QLabel("Send from"), 0, 1)
        self._mf_ph_combos = {}
        mf_ph_cfg = mf_cfg.get("phenomena", {}) if isinstance(mf_cfg.get("phenomena"), dict) else {}
        for row, (key, label) in enumerate(METEO_FRANCE_PHENOMENA.items(), start=1):
            gb_mf_ph_l.addWidget(QtWidgets.QLabel(label), row, 0)
            cb = QtWidgets.QComboBox()
            cb.addItem("Disabled", "disabled")
            cb.addItem("Yellow", "yellow")
            cb.addItem("Orange", "orange")
            cb.addItem("Red", "red")
            wanted = str(mf_ph_cfg.get(key, "orange")).strip().lower()
            idx = cb.findData(wanted)
            cb.setCurrentIndex(idx if idx >= 0 else cb.findData("orange"))
            self._mf_ph_combos[key] = cb
            gb_mf_ph_l.addWidget(cb, row, 1)
        met_l.addWidget(gb_mf_ph)

        test_mf_row = QtWidgets.QHBoxLayout()
        self.btn_mf_api_test = QtWidgets.QPushButton("Test Météo-France API")
        self.btn_mf_api_test.clicked.connect(self._test_meteo_france_api)
        test_mf_row.addWidget(self.btn_mf_api_test)
        self.btn_mf_test = QtWidgets.QPushButton("Test Météo-France DAPNET")
        self.btn_mf_test.clicked.connect(lambda: self._send_dapnet_test_message("meteo_france"))
        test_mf_row.addWidget(self.btn_mf_test)
        test_mf_row.addStretch(1)
        met_l.addLayout(test_mf_row)

        hint_mf = QtWidgets.QLabel(
            "• SUMO sends only new/changed Vigilance alerts, not every refresh.\n"
            "• Use Get API Token to open the official Météo-France API portal.\n"
            "• Official Météo-France API access requires a valid Bearer token.\n"
            "• DAPNET credentials and recipients are taken from the DAPNET tab."
        )
        hint_mf.setStyleSheet("color: #aab6c5; font-size: 11px;")
        hint_mf.setWordWrap(True)
        met_l.addWidget(hint_mf)
        met_l.addStretch(1)
        ...
        tabs.addTab(tab_meteo, "Météo-France")
    ...
    def auto_focus_solar_on_alert(self) -> bool:
        try:
            return bool(self.cb_auto_focus_solar_on_alert.isChecked())
        except Exception:
            return True

    def auto_focus_min_severity(self) -> str:
        try:
            val = str(self.cb_auto_focus_min_severity.currentData() or "ALERT").strip().upper()
        except Exception:
            val = "ALERT"
        return val if val in ("WATCH", "ALERT", "DANGER", "EXTREME") else "ALERT"

    def auto_focus_hold_seconds(self) -> int:
        try:
            return max(10, int(self.sb_auto_focus_hold_seconds.value()))
        except Exception:
            return 60

    def auto_focus_cooldown_minutes(self) -> int:
        try:
            return max(1, int(self.sb_auto_focus_cooldown_minutes.value()))
        except Exception:
            return 10
    ...
    def dapnet_config(self) -> dict:
        cfg = normalize_dapnet_config({
            ...
            "meteo_france": {
                "enabled": bool(self.cb_mf_enabled.isChecked()),
                "api_token": self.ed_mf_api_token.text().strip(),
                "token_created_ts": int(getattr(self, "_mf_token_created_ts", 0) or 0),
                "token_validity_seconds": int(self.sb_mf_token_validity.value()),
                "api_url": self.ed_mf_api_url.text().strip(),
                "departments": self.ed_mf_departments.text().strip(),
                "refresh_minutes": int(self.sb_mf_refresh.value()),
                "emergency_on_red": bool(self.cb_mf_emergency_red.isChecked()),
                "phenomena": {
                    key: str(cb.currentData() or "orange")
                    for key, cb in getattr(self, "_mf_ph_combos", {}).items()
                },
            },
            "iss": {
    ...
    def _test_meteo_france_api(self):
        try:
            cfg = self.dapnet_config()
            mf_cfg = cfg.get("meteo_france", {}) if isinstance(cfg, dict) else {}
            # Convenience: if the token works but no local creation date was saved yet, the user can set it with the dedicated button.
            self.sb.showMessage("Météo-France API test running...", 4000)
            QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
            try:
                payload, status = _mf_fetch_json_detailed(mf_cfg)
            finally:
                try:
                    ...
                    pass
            events = _mf_extract_vigilances(payload)
            deps_watched = _mf_parse_departments(mf_cfg.get("departments"))
            matching = [ev for ev in events if _mf_normalize_department(ev.get("dep")) in deps_watched] if deps_watched else []
            state = load_dapnet_state(DAPNET_STATE_PATH)
            now_iso = datetime.now(timezone.utc).isoformat()
            state["meteo_france_last_check_utc"] = now_iso
            state["meteo_france_last_success_utc"] = now_iso
            state["meteo_france_last_http_code"] = status.get("http_code")
            state["meteo_france_last_json_ok"] = bool(status.get("json_ok"))
            state["meteo_france_last_bulletin_utc"] = status.get("bulletin_ts") or ""
            state["meteo_france_last_events_count"] = status.get("events_count")
            state["meteo_france_last_departments_count"] = status.get("departments_count")
            state["meteo_france_last_error"] = ""
            save_dapnet_state(DAPNET_STATE_PATH, state)
            if hasattr(self, "lb_mf_api_status"):
                self.lb_mf_api_status.setText(_mf_status_from_state())
            if hasattr(self, "_refresh_mf_token_status_ui"):
                self._refresh_mf_token_status_ui()
            msg = (
                "Météo-France API OK\n\n"
                f"HTTP: {status.get('http_code')}\n"
                f"JSON: OK\n"
                f"Downloaded: {status.get('bytes')} bytes\n"
                f"Content-Type: {status.get('content_type') or '?'}\n"
                f"Bulletin timestamp: {status.get('bulletin_ts') or '?'}\n"
                f"Events parsed: {status.get('events_count')}\n"
                f"Departments parsed: {status.get('departments_count')}\n"
                f"Matching watched departments: {len(matching)}"
            )
            QtWidgets.QMessageBox.information(self, "Météo-France API test", msg)
            self.sb.showMessage("Météo-France API test OK", 6000)
        except Exception as exc:
            try:
                QtWidgets.QApplication.restoreOverrideCursor()
            except Exception:
                pass
            state = load_dapnet_state(DAPNET_STATE_PATH)
            state["meteo_france_last_check_utc"] = datetime.now(timezone.utc).isoformat()
            state["meteo_france_last_json_ok"] = False
            state["meteo_france_last_error"] = str(exc)
            save_dapnet_state(DAPNET_STATE_PATH, state)
            if hasattr(self, "lb_mf_api_status"):
                self.lb_mf_api_status.setText(_mf_status_from_state())
            if hasattr(self, "_refresh_mf_token_status_ui"):
                self._refresh_mf_token_status_ui()
            QtWidgets.QMessageBox.critical(self, "Météo-France API test", f"ERROR:\n{exc}")
            self.sb.showMessage(f"Météo-France API test failed: {exc}", 8000)

    def _open_meteo_france_api_portal(self):
        try:
            QtGui.QDesktopServices.openUrl(QtCore.QUrl("https://portail-api.meteofrance.fr/web/fr"))
            self.sb.showMessage("Opening Météo-France API portal...", 5000)
        except Exception as exc:
            try:
                QtWidgets.QMessageBox.warning(self, "Météo-France API portal", f"Unable to open browser:\n{exc}")
            except Exception:
                pass
    ...
    def _send_dapnet_test_message(self, module_name: str = "generic"):
        ...
        if module_key == "xray":
            ...
            text = "ISS TEST 21:45 AZ180 EL62"
        elif module_key in ("meteo_france", "meteo"):
            text = "MF TEST ORANGE ORAGES dep35"
            emergency = False
        else:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        try:
            ...
            if app_icon is not None and not app_icon.isNull():
                ...
                fallback_icon = build_sumo_icon()
                if not fallback_icon.isNull():
                    self.setWindowIcon(fallback_icon)
        except Exception as exc:
            debug_log(f"MainWindow setWindowIcon error: {exc}")
        ...
        # --- Alert focus: temporarily return to the 9 solar-weather tiles when an alert appears ---
        self._auto_focus_solar_on_alert = bool(self._cfg.get("auto_focus_solar_on_alert", True))
        self._auto_focus_min_severity = str(self._cfg.get("auto_focus_min_severity") or "ALERT").strip().upper()
        if self._auto_focus_min_severity not in ("WATCH", "ALERT", "DANGER", "EXTREME"):
            self._auto_focus_min_severity = "ALERT"
        try:
            self._auto_focus_hold_seconds = max(10, int(self._cfg.get("auto_focus_hold_seconds", 60)))
        except Exception:
            self._auto_focus_hold_seconds = 60
        try:
            self._auto_focus_cooldown_seconds = max(60, int(self._cfg.get("auto_focus_cooldown_minutes", 10)) * 60)
        except Exception:
            self._auto_focus_cooldown_seconds = 600
        self._alert_focus_until_monotonic = 0.0
        self._alert_focus_last_monotonic = 0.0
        self._alert_focus_last_signature = ""
    ...
    def _open_settings(self):
        ...
        new_auto_focus_solar_on_alert = dlg.auto_focus_solar_on_alert()
        new_auto_focus_min_severity = dlg.auto_focus_min_severity()
        new_auto_focus_hold_seconds = dlg.auto_focus_hold_seconds()
        new_auto_focus_cooldown_minutes = dlg.auto_focus_cooldown_minutes()
        ...
        changed_display = (
            ...
            bool(new_auto_focus_solar_on_alert) != bool(getattr(self, "_auto_focus_solar_on_alert", True)) or
            str(new_auto_focus_min_severity) != str(getattr(self, "_auto_focus_min_severity", "ALERT")) or
            int(new_auto_focus_hold_seconds) != int(getattr(self, "_auto_focus_hold_seconds", 60)) or
            int(new_auto_focus_cooldown_minutes) != int(getattr(self, "_auto_focus_cooldown_seconds", 600) // 60) or
        ...
        self._auto_focus_solar_on_alert = bool(new_auto_focus_solar_on_alert)
        self._auto_focus_min_severity = str(new_auto_focus_min_severity).strip().upper() or "ALERT"
        if self._auto_focus_min_severity not in ("WATCH", "ALERT", "DANGER", "EXTREME"):
            self._auto_focus_min_severity = "ALERT"
        self._auto_focus_hold_seconds = max(10, int(new_auto_focus_hold_seconds))
        self._auto_focus_cooldown_seconds = max(60, int(new_auto_focus_cooldown_minutes) * 60)
        ...
        self._cfg["auto_focus_solar_on_alert"] = bool(getattr(self, "_auto_focus_solar_on_alert", True))
        self._cfg["auto_focus_min_severity"] = str(getattr(self, "_auto_focus_min_severity", "ALERT"))
        self._cfg["auto_focus_hold_seconds"] = int(getattr(self, "_auto_focus_hold_seconds", 60))
        self._cfg["auto_focus_cooldown_minutes"] = max(1, int(getattr(self, "_auto_focus_cooldown_seconds", 600) // 60))
    ...
    def _effective_main_display(self) -> str:
        ...
        try:
            if current == "solar" and time.monotonic() < float(getattr(self, "_alert_focus_until_monotonic", 0.0)):
                return "solar"
        except Exception:
            pass
    ...
    def _update_display_rotation(self, force_reset: bool = False):
        ...
        # During an alert-focus hold, keep the 9 solar-weather tiles visible and do not rotate.
        try:
            if previous == "solar" and now_mono < float(getattr(self, "_alert_focus_until_monotonic", 0.0)):
                return False
        except Exception:
            pass
    ...
    # =====================================================
    # Alert focus helper: return to the 9 solar-weather tiles when an alert appears
    # =====================================================
    def _maybe_auto_focus_solar_on_alert(self, key: str, family: str, prev: str, sev: str) -> None:
        try:
            if not bool(getattr(self, "_auto_focus_solar_on_alert", True)):
                return
            if str(family).strip().lower() != "risk":
                return
            key = str(key or "?").strip().upper()
            prev = str(prev or SEV_UNKNOWN).strip().upper()
            sev = str(sev or SEV_UNKNOWN).strip().upper()
            min_sev = str(getattr(self, "_auto_focus_min_severity", "ALERT") or "ALERT").strip().upper()
            if min_sev not in RISK_ORDER:
                min_sev = SEV_ALERT
            min_i = _sev_index(RISK_ORDER, min_sev)
            prev_i = _sev_index(RISK_ORDER, prev)
            now_i = _sev_index(RISK_ORDER, sev)

            # Trigger only when a panel enters or crosses the configured alert level.
            if now_i < min_i or prev_i >= min_i:
                return

            now_mono = time.monotonic()
            cooldown = max(60.0, float(getattr(self, "_auto_focus_cooldown_seconds", 600)))
            if now_mono - float(getattr(self, "_alert_focus_last_monotonic", 0.0)) < cooldown:
                return

            hold = max(10.0, float(getattr(self, "_auto_focus_hold_seconds", 60)))
            previous_view = self._effective_main_display()
            self._display_current_view = "solar"
            self._display_last_switch_monotonic = now_mono
            self._alert_focus_until_monotonic = now_mono + hold
            self._alert_focus_last_monotonic = now_mono
            self._alert_focus_last_signature = f"{key}:{sev}"
            if previous_view != "solar":
                self._dbg(f"Alert focus: {key} {prev}->{sev}, switching {previous_view} -> solar for {int(hold)}s", hardcore_only=False)
                self._layout_main_grid()
            else:
                self._dbg(f"Alert focus: {key} {prev}->{sev}, solar hold refreshed for {int(hold)}s", hardcore_only=True)
        except Exception as exc:
            try:
                self._dbg(f"Alert focus failed: {exc}", hardcore_only=True)
            except Exception:
                pass
    ...
    def _set_panel_severity(self, key: str, family: str, sev: str, blink_on_danger: bool = True):
        ...
        try:
            self._maybe_auto_focus_solar_on_alert(key, family, prev, sev)
        except Exception:
            pass
...
def _dapnet_send_message(self, module: str, text: str, *, emergency: bool = False, state_key: str | None = None, cooldown_seconds: int | None = None) -> tuple[bool, str]:
    try:
        module_name = str(module or "").strip().upper()
        if cooldown_seconds is None and module_name.startswith("ISS"):
            cooldown_seconds = DAPNET_ISS_COOLDOWN
        ok, resp = self._dapnet_client.send_message(text, emergency=emergency, cooldown_seconds=cooldown_seconds)
    except Exception as exc:
...
def _dapnet_compute_next_iss_pass(self, now_utc: datetime | None = None) -> dict | None:
    """Compute next ISS pass for DAPNET.

    v7.9.8 fix:
    - The ISS TLE/Skyfield objects live in ``self.solar_system_dashboard``.
      The previous DAPNET code looked for ``self._iss_satellite`` on MainWindow,
      so it always returned None and no ISS DAPNET alert could be emitted.
    - Uses Skyfield ``find_events`` instead of a coarse 60 s scan.
    """
    ...
    try:
        ...
        # MainWindow does not own the ISS satellite; SolarSystemDashboard does.
        dash = getattr(self, 'solar_system_dashboard', None)
        sat = getattr(dash, '_iss_satellite', None) if dash is not None else None
        ts = getattr(dash, '_skyfield_ts', None) if dash is not None else None

        # Keep a small fallback in case a future build moves Skyfield objects back to MainWindow.
        if sat is None:
            sat = getattr(self, '_iss_satellite', None)
        if ts is None:
            ts = getattr(self, '_skyfield_ts', None)

        if sat is None:
            try:
                tle_status = getattr(dash, '_iss_tle_error', '') if dash is not None else 'no dashboard'
                self._dbg(f'DAPNET ISS skipped: satellite unavailable ({tle_status})')
            except Exception:
                pass
            return None
        if ts is None or wgs84 is None:
            try:
                self._dbg('DAPNET ISS skipped: Skyfield timescale/wgs84 unavailable')
            except Exception:
                pass
            return None

        observer = wgs84.latlon(
            float(cfg.get('observer_lat', 48.1173)),
            float(cfg.get('observer_lon', -1.6778)),
            elevation_m=float(cfg.get('observer_alt_m', 60)),
        )

        t0 = ts.from_datetime(now_utc)
        t1 = ts.from_datetime(now_utc + timedelta(hours=12))
        times, events = sat.find_events(observer, t0, t1, altitude_degrees=min_elev)
        ...
        for t, event in zip(times, events):
            ...
            if ev == 0:
                ...
                try:
                    ...
                    alt, az, distance = topocentric.altaz()
                    current['aos_az_deg'] = float(az.degrees) % 360.0
                    current['aos_elev_deg'] = float(alt.degrees)
                except Exception:
                    current['aos_az_deg'] = None
                    current['aos_elev_deg'] = None
            elif ev == 1 and current:
                current['max'] = dt
                try:
                    topocentric = (sat - observer).at(t)
                    alt, az, distance = topocentric.altaz()
                    current['max_elev_deg'] = float(alt.degrees)
                    current['max_az_deg'] = float(az.degrees) % 360.0
                except Exception:
                    current['max_elev_deg'] = None
                    current['max_az_deg'] = None
            elif ev == 2 and current:
                current['los'] = dt
                try:
                    topocentric = (sat - observer).at(t)
                    alt, az, distance = topocentric.altaz()
                    current['los_az_deg'] = float(az.degrees) % 360.0
                    current['los_elev_deg'] = float(alt.degrees)
                except Exception:
                    current['los_az_deg'] = None
                    current['los_elev_deg'] = None
                passes.append(current)
                current = {}

        if not passes:
            try:
                self._dbg(f'DAPNET ISS: no pass > {min_elev:.0f}° in next 12h')
            except Exception:
                pass
            return None
        ...
        p = passes[0]
        start_dt = p.get('aos')
        peak_dt = p.get('max') or start_dt
        end_dt = p.get('los')
        if not isinstance(start_dt, datetime) or not isinstance(end_dt, datetime):
            return None
        peak_el = p.get('max_elev_deg')
        try:
            peak_el = float(peak_el) if peak_el is not None else float(min_elev)
        except Exception:
            peak_el = float(min_elev)
        ...
        result = {
            ...
            'start_az_deg': _safe_round(p.get('aos_az_deg')),
            'start_elev_deg': _safe_round(p.get('aos_elev_deg'), min_elev),
            'peak_az_deg': _safe_round(p.get('max_az_deg')),
            'peak_elev_deg': round(float(peak_el), 1),
            'end_az_deg': _safe_round(p.get('los_az_deg')),
            'end_elev_deg': _safe_round(p.get('los_elev_deg'), min_elev),
        }
        try:
            self._dbg(
                'DAPNET ISS next pass: '
                f"AOS={result['start_utc']} peak={result['peak_utc']} "
                f"LOS={result['end_utc']} max={result['peak_elev_deg']}°"
            )
        except Exception:
            pass
        return result
...
def _dapnet_fmt_deg3(value) -> str:
    """Format azimuth as 3 digits without degree symbol for pager safety."""
    try:
        v = int(round(float(value))) % 360
        return f"{v:03d}"
    except Exception:
        return "---"


def _dapnet_fmt_el2(value) -> str:
    """Format elevation as 2 digits without degree symbol for pager safety."""
    try:
        v = max(0, min(90, int(round(float(value)))))
        return f"{v:02d}"
    except Exception:
        return "--"


def _dapnet_iss_msg(kind: str, hhmm: str, az_deg, el_deg) -> str:
    """Clear ISS DAPNET message, one event per message.

    Format kept short and consistent for 80-character pagers:
    - ISS bientot 18:59 AZ315 EL05
    - ISS debut 18:59 AZ315 EL05
    - ISS max 19:03 AZ182 EL64
    - ISS fin 19:07 AZ078 EL05
    """
    ...
    if label == "prealert":
        ...
        word = label or "info"
    return _dapnet_compact_text(["ISS", word, str(hhmm or "--:--"), f"AZ{_dapnet_fmt_deg3(az_deg)}", f"EL{_dapnet_fmt_el2(el_deg)}"])
...
def _dapnet_check_iss(self, now_utc: datetime) -> None:
    ...
    start_az = pass_info.get('start_az_deg', 0.0)
    start_el = pass_info.get('start_elev_deg', cfg.get('min_elevation', 5.0))
    peak_az = pass_info.get('peak_az_deg', 0.0)
    peak_el = float(pass_info.get('peak_elev_deg', 0.0) or 0.0)
    end_az = pass_info.get('end_az_deg', 0.0)
    end_el = pass_info.get('end_elev_deg', cfg.get('min_elevation', 5.0))
    ...
    _maybe('prealert', bool(cfg.get('prealert_enabled', True)), prealert_dt,
           _dapnet_iss_msg('prealert', _dapnet_format_local(self, start_dt), start_az, start_el))
    _maybe('start', bool(cfg.get('start_enabled', True)), start_dt,
           _dapnet_iss_msg('start', _dapnet_format_local(self, start_dt), start_az, start_el))
    _maybe('peak', bool(cfg.get('peak_enabled', True)), peak_dt,
           _dapnet_iss_msg('peak', _dapnet_format_local(self, peak_dt), peak_az, peak_el))
    _maybe('end', bool(cfg.get('end_enabled', True)), end_dt,
           _dapnet_iss_msg('end', _dapnet_format_local(self, end_dt), end_az, end_el))
...
def _process_dapnet_from_payload(self, payload: dict | None) -> None:
    ...
    _process_meteo_france_vigilance(self, cfg, state, now_utc)
...
def _mf_department_sort_key(dep: str):
    """Sort French departments naturally: 01..95, 2A, 2B, 971..."""
    d = _mf_normalize_department(dep)
    if d == "2A":
        return (2, 1, d)
    if d == "2B":
        return (2, 2, d)
...
def _mf_chunk_departments_for_dapnet(prefix: str, departments: list[str], max_len: int = 80) -> list[list[str]]:
    """Split department lists so each DAPNET message stays under 80 chars.

    The final message format is: "<prefix> 22,29,35".
    Most regional selections fit in one message, but national selections are
    split cleanly without losing the grouping logic.
    """
    clean = [_mf_normalize_department(d) for d in departments]
    clean = [d for d in clean if d]
    chunks: list[list[str]] = []
    current: list[str] = []
    safe_prefix = str(prefix or "MF").strip()

    for dep in clean:
        trial = current + [dep]
        trial_text = f"{safe_prefix} {','.join(trial)}".strip()
        if current and len(trial_text) > int(max_len):
...
def _process_meteo_france_vigilance(self, cfg: dict, state: dict, now_utc: datetime) -> None:
    try:
        mf_cfg = cfg.get("meteo_france", {}) if isinstance(cfg, dict) else {}
        if not bool(mf_cfg.get("enabled", False)):
            return
        if not bool(cfg.get("enabled", False)):
            return
        deps = _mf_parse_departments(mf_cfg.get("departments"))
        if not deps:
            return
        last_dt = _dapnet_parse_iso8601(state.get("meteo_france_last_check_utc"))
        refresh = max(5, int(mf_cfg.get("refresh_minutes", 5)))
        if last_dt is not None and (now_utc - last_dt) < timedelta(minutes=refresh):
            return
        state["meteo_france_last_check_utc"] = now_utc.isoformat()

        payload, mf_status = _mf_fetch_json_detailed(mf_cfg, logger=getattr(self, "_dbg", None))
        events = _mf_extract_vigilances(payload)
        state["meteo_france_last_success_utc"] = now_utc.isoformat()
        state["meteo_france_last_http_code"] = mf_status.get("http_code")
        state["meteo_france_last_json_ok"] = bool(mf_status.get("json_ok"))
        state["meteo_france_last_bulletin_utc"] = mf_status.get("bulletin_ts") or ""
        state["meteo_france_last_events_count"] = mf_status.get("events_count")
        state["meteo_france_last_departments_count"] = mf_status.get("departments_count")
        state["meteo_france_last_error"] = ""
        ph_cfg = mf_cfg.get("phenomena", {}) if isinstance(mf_cfg.get("phenomena"), dict) else {}
        sent_map = state.setdefault("meteo_france_sent", {})
        active_now = set()

        # Group DAPNET messages by échéance + couleur + phénomène.
        # Example output:
        #   MF J0 ORANGE ORAGES 22,35,56
        #   MF J+1 JAUNE VENT VIOLENT 29,35
        # If the department list would exceed the 80 char DAPNET limit, SUMO
        # automatically splits the group into several compact messages.
        grouped: dict[tuple[str, int, str], set[str]] = {}
        for ev in events:
            dep = _mf_normalize_department(ev.get("dep"))
            if dep not in deps:
                continue
            ph = _mf_normalize_phenomenon(ev.get("phenomenon"))
            if ph not in METEO_FRANCE_PHENOMENA:
                continue
            level = _mf_level_to_int(ev.get("level"))
            threshold = _mf_threshold_to_int(ph_cfg.get(ph, "orange"))
            if level < threshold or threshold >= 99:
                continue
            deadline = _mf_normalize_deadline(ev.get("deadline", "J0"))
            grouped.setdefault((deadline, level, ph), set()).add(dep)

        for (deadline, level, ph), dep_set in sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1], item[0][2])):
            level_name = _mf_level_name(level)
            ph_label = METEO_FRANCE_PHENOMENA.get(ph, ph).upper()
            emergency = bool(mf_cfg.get("emergency_on_red", True)) and level >= 4
            prefix = _dapnet_compact_text(["MF", deadline, level_name, ph_label])
            dep_chunks = _mf_chunk_departments_for_dapnet(prefix, sorted(dep_set, key=_mf_department_sort_key))
            for chunk in dep_chunks:
                deps_txt = ",".join(chunk)
                event_key = f"{deadline}|{level}|{ph}|{deps_txt}"
                active_now.add(event_key)
                if sent_map.get(event_key):
                    continue
                msg = _dapnet_compact_text([prefix, deps_txt])
                ok, _ = _dapnet_send_message(self, f"MF/{deadline}/{level_name}/{ph}", msg, emergency=emergency)
                if ok:
                    sent_map[event_key] = now_utc.isoformat()

        # Garbage collect old entries no longer active to allow resend if alert disappears then returns,
        # or if a grouped department list changes after a bulletin update.
        ...
        try:
            state["meteo_france_last_error"] = str(exc)
            self._dbg(f"Météo-France Vigilance failed: {exc}")
        except Exception:
            pass
...
MainWindow._process_meteo_france_vigilance = _process_meteo_france_vigilance
